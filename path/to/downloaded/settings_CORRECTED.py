...
class Settings:
    ...
    def __init__(self, **overrides):
        ...
        for key, value in overrides.items():
            if key.endswith('_PORT') or key.endswith('_SIZE') or key == 'MAX_WORKERS' or key == 'API_WORKERS':
                self._overrides[key] = int(value) if not isinstance(value, int) else value
            elif key.startswith('DEBUG_') or key in ['AI_ENABLED', 'MODULE_AUTO_LOAD', 'CACHE_ENABLED', 'AI_FALLBACK_TO_EMULATOR']:
