...
class MockTreeFramework(ITreeFramework):
    ...
    def build_tree(self, module_id: str, nodes: List[LogicTreeNode]) -> None:
        """
    ...
    def query_tree(self, module_id: str, filters: Dict[str, Any] = None) -> List[LogicTreeNode]:
        """
    ...
    def _validate_tree_structure(self, nodes: List[LogicTreeNode]) -> Tuple[bool, List[str]]:
        """
    ...
    def _matches_filters(self, node: LogicTreeNode, filters: Dict[str, Any]) -> bool:
        """
...
class MockAnalysisEngine(IAnalysisEngine):
    ...
    async def analyze(
        self, module: ILegalModule, filled_fields: Dict[str, Any], enhance_with_ai: bool = True
    ) -> Dict[str, Any]:
        """
        ...
        calculation_result = await module.calculate(
            matched_nodes=tree_nodes[:2], filled_fields=filled_fields  # Use first 2 nodes
        )
        ...
        if enhance_with_ai:
            result["ai_insights"] = self._generate_mock_ai_insights(
                filled_fields, calculation_result
            )
